...
func main() {
	...
	var svc pb.UserServer = service.UserService{}
	// 依賴注入 service
	...
	r := transport.MakeHTTPHandler(ctx, endpts, localconfig.ZipkinTracer, localconfig.Logger, localconfig.Level)

	// http server
	go func() {
		localconfig.Logger.Info(fmt.Sprintf("Http Server start at port: %v, ", *servicePort))
		// 初始化 Mysql
	...
	// grpc server
	go func() {
		localconfig.Logger.Info(fmt.Sprintf("grpc Server start at port: %v, ", *grpcAddr))
	...
	// 服務關閉後，服務註銷
	discover.Deregister()
	localconfig.Logger.Error(fmt.Sprintf("%v", error))
}
